...
contract('Stake', function([admin1, admin2, parent, flux, p1, p2]) {
    ...
    const three = web3.toBigNumber(3);
    const four = web3.toBigNumber(4);
    ...
    describe("Visibility restrictions", () => {
        it("onlyAdmin correct", async function() {
            const t = await this.stakeData.token().should.be.fulfilled;
            console.log(t);
            const c = await this.stakeData.getUcacAddr(web3.fromAscii("hi"), {from: this.stake.address}).should.be.fulfilled;
            console.log(c);
        ...
        it("onlyFlux correct", async function() {
            ...
            // await this.stake.createAndStakeUcac(flux, flux, good_ucacId1, h.toWei(10000), {from: p1}).should.be.fulfilled;
