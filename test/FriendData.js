...
contract('FriendData', function([admin1, admin2, parent, p1, p2]) {
    ...
    describe("Visibility restrictions", () => {
        ...
        it("Only parent contract can call setters", async function() {
            ...
            await this.fd.initFrienship(ucacMain, id1, id2, {from: p1}).should.be.rejectedWith(h.EVMThrow);
        });
        
        // private modifier
        it("Private function only available internally", async function() {
            await this.fd.friendIndices(ucacMain, id1, id2, {from: admin1}).should.be.rejectedWith(h.EVMThrow);
            await this.fd.friendIndices(ucacMain, id1, id2, {from: parent}).should.be.rejectedWith(h.EVMThrow);
        });
    ...
    describe("Value setting and getting", () => {
        ...
        it("Individual setters and getters function correctly", async function() {
            ...
            (await this.fd.friendships[ucacMain][id1][id2].initialized).valueOf().should.equal(false);
            (await this.fd.friendships[ucacMain][id2][id1].initialized).valueOf().should.equal(true);
            ...
            (await this.fd.friendships[ucacMain][id1][id2].initialized).valueOf().should.equal(false);
            (await this.fd.friendships[ucacMain][id2][id1].initialized).valueOf().should.equal(false);
            
            await this.fd.fSetInitialized(ucacMain, id1, id2, true, {from: parent}).should.be.fulfilled;
            (await this.fd.fInitialized(ucacMain, id1, id2)).valueOf().should.equal(true);
            (await this.fd.fInitialized(ucacMain, id2, id1)).valueOf().should.equal(true);
            (await this.fd.friendships[ucacMain][id1][id2].initialized).valueOf().should.equal(true);
            (await this.fd.friendships[ucacMain][id2][id1].initialized).valueOf().should.equal(false);
